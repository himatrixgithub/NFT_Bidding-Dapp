...
const NFT_ADDRESS = "0xF0c75654c68FE44fE639f1419F126Ff4aD4E5e2F"; // NFT contract address
const AUCTIONMANAGER_ADDRESS = "0xd4d9E3504040aB94351323e8e642ec9ea3756ee1"; // AuctionManager contract address
class App extends React.Component {
    ...
    do {
      try {
        ...
        if (tokenOwner === walletAddress) {
          NFTIds.push(counter);
          tokenuri = await this._nft.tokenURI(counter);
          uri.push(tokenuri);
        }
        counter++;
      } catch (error) {
  ...
  async createAuction() {
    if (
      ...
      !this.state.newAuction.tokenId.toString()
    ) {
  ...
  async getAuctions() {
    ...
    for (let i = 0; i < auctions.endTime.length; i++) {
      ...
      // console.log(await this._nft.tokenURI(tokenId));


      let text = await this._nft.tokenURI(tokenId);
      let result;
      result = text.slice(6);
      ...
      try {
        ...
        // console.log(imageUrl); // Access imageUrl inside the loop
      } catch (error) {
  ...
  renderAuctionElement(auction, _this) {
    ...
    return (
      <div style={{ background: backgroundColor }} class=" color-box">
        ...
        <p className="photo">
          <img src={auction.uri} alt="unable to load" className="photo"></img>
        </p>
        <button
          class="btn-primary"
          onClick={() => {
  ...
  renderActiveAuction() {
    ...
    return (
      <div>
          ...
          <div class="color-box-in" style={{ backgroundColor: "#D1F3FD" }}>
            ...
            <div image-div>
              <p className="photo">
                <img src={activeAuction.uri} alt="unable to load" className="photo"></img>
              </p>
            </div>
      ...
      <div>
        ...
        <div class="container">
          <form>
            ...
            <a href="https://avax-project20022.web.app" class="btn btn-primary" target="_blank" rel="noreferrer">
